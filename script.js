...
i18n.ar.no_expired='لا يوجد تذكيرات منتهية';
i18n.en.no_expired='No expired reminders';
const state={reminders:[],map:new Map(),expiredMap:new Map(),lang:localStorage.getItem('lang')||'ar'};
let uiBound=false;
...
const uploadImages=async(files)=>{if(!files||!files.length)return[];const key=(window.imgbbKey||localStorage.getItem('imgbbKey')||'').trim();const expiration=(window.imgbbExpiration||'');const b64=await toBase64s(files);const durls=await toDataUrls(files);if(!key)return durls;const urls=[];for(let i=0;i<b64.length;i++){try{const fd=new FormData();fd.append('image',b64[i].base64);fd.append('name',b64[i].name||('img-'+Date.now()));const endpoint='https://api.imgbb.com/1/upload?key='+encodeURIComponent(key)+(expiration?('&expiration='+encodeURIComponent(expiration)):'');const res=await fetch(endpoint,{method:'POST',body:fd});if(!res.ok)throw new Error('upload');const data=await res.json();const u=(data&&data.data&&(data.data.display_url||data.data.url))||null;urls.push(u||durls[i])}catch(_){urls.push(durls[i])}}return urls}; 
...
const dbOps={init:async()=>{},getAll:async()=>{if(!fsdb)return [];const snap=await fsdb.collection('reminders').orderBy('dueAt','asc').get();return snap.docs.map(d=>d.data())},add:async(obj)=>{if(!fsdb)return;await fsdb.collection('reminders').doc(String(obj.id)).set(obj,{merge:true})}};
const typeLabel=v=>{const mapAr={'اختبار شهري':'type_exam','اسايمنت':'type_assignment','ريبورت':'type_report','برزنتيشن/عرض تقديمي':'type_presentation','اختبار نهائي':'type_final'};const key=mapAr[v]||null;return key?i18n[state.lang][key]:(v||'')};
const render=()=>{
  ...
  const renderGroup=(items,rootList,isExpired)=>{
    ...
    groups.forEach((gi)=>{
      ...
      gi.forEach(rem=>{
        ...
        images.innerHTML=(rem.images||[]).map(src=>`<img src="${src}">`).join('');
...
let popoverEl=null,overlayEl=null;
const openPop=()=>{popoverEl?.classList.remove('hidden');overlayEl?.classList.remove('hidden')};
...
const bindPopover=()=>{popoverEl=$('#addPopover');overlayEl=$('#overlay');const btn=$('#openAdd');if(btn&&!btn.dataset.bound){btn.dataset.bound='1';btn.addEventListener('click',()=>openPop())}overlayEl?.addEventListener('click',()=>closePop());document.addEventListener('keydown',e=>{if(e.key==='Escape')closePop()})};
...
const boot=async()=>{await dbOps.init();applyI18n();applySubjectLabel();if(!uiBound){bindForm();bindPopover();bindLangIcon();uiBound=true}initFirebase();await authReady;startRealtime();state.reminders=await dbOps.getAll();render();setInterval(tick,1000)};
